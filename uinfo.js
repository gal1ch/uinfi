try {
	...
	try {
		if (OS_Type == 1) {
			query_printers = wmi.ExecQuery('SELECT Name,Default,Network,ServerName,ShareName FROM Win32_Printer WHERE (NOT Name LIKE "%Microsoft%" AND NOT Name LIKE "%Fax%" AND NOT Name LIKE "%XPS%" AND NOT Name LIKE "%ิเ๊๑%" AND NOT Name LIKE "%PDF%" AND NOT Name LIKE "%OneNote%")');	
